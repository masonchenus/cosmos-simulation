...
const WebGLRenderer = {
    ...
    // Visualization settings - optimized for simple browsers
    settings: {
        showOrbits: false, // Disabled by default for performance
        showLabels: false, // Disabled by default for performance
        showAsteroids: false, // Disabled by default for performance
        showMoons: true,
        scalePlanets: 0.0001, // Scale factor for planet sizes
        scaleDistances: 1, // Scale factor for distances
        orbitOpacity: 0.3,
        labelSize: 14,
        starFieldDensity: 500, // Minimal stars for simple browsers
        asteroidCount: 100 // Minimal asteroids for simple browsers
    },
    ...
    /**
     * Create the Sun with glow effect - simplified for simple browsers
     */
    createSun: function () {
        const sunData = AstronomicalData.Sun;

        // Sun mesh - minimal segments for simple browsers
        ...
        const coronaGeometry = new THREE.SphereGeometry(8, 12, 12);
    ...
    /**
     * Create a single planet with orbit - simplified for simple browsers
     */
    createPlanet: function (name, data) {
        // Create planet mesh - minimal segments for simple browsers
        var size = data.radius * this.settings.scalePlanets;
        var geometry = new THREE.SphereGeometry(Math.max(size, 0.1), 12, 12);
    ...
    createOrbit: function (data, name) {
        var points = [];
        var segments = 128; // Reduced for performance
    ...
    /**
     * Create a moon with orbit - simplified for simple browsers
     */
    createMoon: function (name, data) {
        var size = Math.max(data.radius * this.settings.scalePlanets * 5, 0.05);
        var geometry = new THREE.SphereGeometry(size, 8, 8); // Minimal for simple browsers
        var material = new THREE.MeshStandardMaterial({
            color: data.color,
            roughness: 0.9,
            metalness: 0.1
